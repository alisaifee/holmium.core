...
class WaitConditionTests(unittest.TestCase):
    ...
    def run_script(self, script, delay=0):
        hiro.run_async(1, lambda: time.sleep(delay) or self.driver.execute_script(script))
    ...
    def test_only_if_displayed(self):
        ...
        self.run_script('document.getElementById("simple_id").style.display="none";')
        self.assertTrue(page.id_el is None)
        self.run_script('document.getElementById("simple_id").style.display="block";', 1)
